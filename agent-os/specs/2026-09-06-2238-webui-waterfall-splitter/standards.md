# Standards for the Waterfall Splitter

Three standards from `agent-os/standards/` apply. **None of them was written for
`argus-webui`** — the standards tree is entirely Kotlin/KMP. Where a rule is language-bound,
only its intent transfers, and the package's own conventions govern the rest:

- `*.states.ts` purity — DOM-free presentation logic in a sibling module, because Vitest runs
  in the default node environment with no jsdom (`EventList.states.ts:1-4`).
- Token lint — `argus-webui/scripts/lint-tokens.ts` fails the build on raw hex or px literals
  outside `src/design/**`, `src/styles/globals.css`, `src/assets/**`.
- Probe harness — real-DOM behaviour is asserted by committed Playwright probes in
  `scripts/probe-webui/`, run in CI by `.github/workflows/verify-webui.yml`.

---

## workflow/commit-conventions

**Applies because:** this work produces commits against `lynxal/KMM-Argus`. `fix:` is the type
(the pane clipping is a bug); the splitter rides along in the same change, referenced `(#31)`.
The no-agent-attribution rule and explicit staging both bind here.
...
**Applies because:** this codebase already documents *why* heavily — see the pin-lock comment
in `virtual.ts:77-97` or the hover-specificity note in `Row.ts:34-37`. Every non-obvious choice
in this change (why the correlation cell stays shrinkable, why narrow mode is an ancestor
attribute rather than row classes, why the resize observer uses `peek()`) needs the same
treatment. KDoc maps to TSDoc.
...
**Applies because:** the new `SplitView.states.test.ts` follows it in shape — one behaviour per
`it`, arrange/act/assert. Kotlin specifics (kotlin.test, backtick names, `@BeforeTest`,
`commonTest`) do not transfer; the Vitest equivalent is the existing
`EventList.states.test.ts`, which this test mirrors exactly.
